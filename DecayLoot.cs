...
namespace Oxide.Plugins
{
    ...
    public class DLoot : RustPlugin
    {
        ...
        void LoadAllContainers()
        {
            ...
            {
                var loot = GameManager.server.FindPrefab(prefab.id)?.GetComponent<LootContainer>();
                foreach (ConfigData.Prefab.Item item in prefab.items)
                {
        ...
        bool PopulateContainer(LootContainer container, ConfigData.Prefab prefab)
        {
            ...
            int itemsCount;
            var rng = new Random();
            if (prefab.item_range[0] == prefab.item_range[1])
                itemsCount = prefab.item_range[1];
            else
                itemsCount = rng.Next(prefab.item_range[0], prefab.item_range[1]);
            /* Prime crates for filling */
            ...
            for (var i = 0; i < (itemsCount > 0 ? itemsCount : 1) - 1; i++)
            {
                ...
                if (!sample.Remove(itemSpec))
                    PrintWarning("error removing item from sample: {0}", itemSpec.shortname);
